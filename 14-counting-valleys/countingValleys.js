...
const countingValleysSlow = (n, s) => {
  let curElevation = 0, curType = '', curIndex = 0, valleyCount = 0;
  while (curIndex < n) {

    // evaluate if we’re in a valley, on a mountain, or finishing either

    if ( curElevation < 0 ) { curType = 'Valley' }
    else if (curElevation > 0) { curType = 'Mountain' }
    else {
      if (curType === 'Valley') { valleyCount += 1 }
      curType = '';
    }
